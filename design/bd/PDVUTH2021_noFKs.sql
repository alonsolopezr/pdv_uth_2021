...
CREATE TABLE IF NOT EXISTS `db_pdvuth2021`.`products` (
  ...
  INDEX `fk_subcategory_id_idx` (`subcategory_id` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `db_pdvuth2021`.`sales` (
  ...
  `ticket_number` INT ZEROFILL UNSIGNED NULL DEFAULT 0000001000000001,
...
CREATE TABLE IF NOT EXISTS `db_pdvuth2021`.`inventory_existence` (
  ...
  UNIQUE INDEX `_UNIQUE` (`product_id` ASC) VISIBLE,
  INDEX `fk__last_inventory_movement_id_idx` (`last_inventory_movement_id` ASC) VISIBLE)
ENGINE = InnoDB;
